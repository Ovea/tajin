...
(function (w, $) {
    "use strict";
    var fix_locale = function (locale) {
        ...
        extensions = function (locale, variants) {
            var tries = [], l;
            // load base
            tries.push('');
        ...
        I18NModule = function () {
            ...
            Bundle.prototype = {
                ...
                localize: function (expr) {
                    ...
                    var self = this,
                        ...
                        dolocEl = function () {
                            var elem = $(this), key = elem.attr("data-i18n"), v = self.value(key);
                            if (tajin.options.i18n.debug) {
                        ...
                        dolocAttr = function (attr) {
                            var elem = $(this), key = elem.attr(attr).match(/i18n\[([\.\w]+)\]/)[1], v = self.value(key);
                            if (tajin.options.i18n.debug) {
                                console.log('[tajin.i18n] localize', self.name, self.locale, key, v, elem);
                            }
                            elem.attr(attr, v);
                    ...
                    $.each(attrs, function (i, attr) {
                        if (e.attr(attr) && e.attr(attr).match(/^i18n/)) {
                            dolocAttr.call(e, attr);
                        }
                        e.find('[' + attr + '^="i18n"]').each(function () {
            ...
            m.oninstall = function (t) {
                tajin = t;
                events = {
                    bundle: tajin.event.on('i18n/bundle/loaded'),
                    html: tajin.event.on('i18n/html/loaded'),
                    image: tajin.event.on('i18n/image/loaded')
                };
            };
                ...
                var b, v = 0, bnds = [], pre = function () {
                    if (b < bnds.length) {
                        ...
                        if (v >= variants.length) {
                            ...
                            load_bundle(bnds[b], variants[v], function (name, locale, incache, b, l) {
                                var bnd = new Bundle(name, locale, b, l);
                                tajin.event.on('i18n/bundle/loaded/' + name).fire(bnd);
