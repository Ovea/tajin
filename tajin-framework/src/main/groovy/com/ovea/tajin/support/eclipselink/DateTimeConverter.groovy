...
class DateTimeConverter implements Converter {
    ...
    public Object convertObjectValueToDataValue(Object objectValue, Session arg1) {
        ...
        if (objectValue instanceof DateTime) {
            return new Timestamp(((DateTime) objectValue).millis)
        } else
            throw new IllegalStateException("Converstion exception, value is not of type org.joda.time.DateTime")
    ...
    void initialize(DatabaseMapping mapping, Session session) {
        if (mapping.directToFieldMapping) {
            ...
            if (directMapping.getFieldClassification() == null) {
                directMapping.setFieldClassification(ClassConstants.TIMESTAMP)
            }
        }
    }
}
