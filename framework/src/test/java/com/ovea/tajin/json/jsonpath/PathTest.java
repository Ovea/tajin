...
public class PathTest {
    ...
    public void path_is_not_definite() throws Exception {
        assertFalse(JSON.compile("$..book[0]").isPathDefinite());
        assertFalse(JSON.compile("$book[?]", filter).isPathDefinite());
        assertFalse(JSON.compile("$.books[*]").isPathDefinite());
    }

    @Test
    public void path_is_definite() throws Exception {
        assertTrue(JSON.compile("$.definite.this.is").isPathDefinite());
        assertTrue(JSON.compile("rows[0].id").isPathDefinite());
    }
